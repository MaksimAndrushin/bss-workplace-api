...
func (s *GrpcServer) Start(ctx context.Context, cfg *config.Config) error {
	...
	redisClient, err := redis.NewRedisClient(cfg.Redis.Host, cfg.Redis.Port)
	if err != nil {
		logger.FatalKV(ctx, "Failed running redis server", "err", err)
	}

	pb.RegisterBssWorkplaceApiServiceServer(grpcServer, api.NewWorkplaceAPI(workplaceRepo, workplaceEventsRepo, s.db, redisClient))
