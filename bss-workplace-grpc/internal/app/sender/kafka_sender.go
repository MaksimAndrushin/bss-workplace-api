...
func NewKafkaSender(brokers []string, topic string, numRetries uint, retryDelaySec uint) (*KafkaSender, error) {
	syncProducer, err := kafka.NewSyncProducer(brokers, numRetries, retryDelaySec)
	if err != nil {
		return nil, err
	}
