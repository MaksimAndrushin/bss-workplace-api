...
func (r *workplaceRepo) ListWorkplaces(ctx context.Context, offset uint64, limit uint64) ([]model.Workplace, uint64, error) {
	...
	count, err := r.CountWorkplaces(ctx)
	if err != nil {
		return nil, 0, err
	}

	return workplaces, count, err
}

func (r *workplaceRepo) CountWorkplaces(ctx context.Context) (uint64, error) {
	span := tracer.CreateSpan(ctx, "Workplace Repo CountWorkplaces")
	defer span.Close()
